...
// ─── Simulation Engine (Eqs. 9, 14 and algebraic coupling, Corollary 3.7) ───
...
const DEFAULT_PARAMS = {
  ...
  // Information-geometric parameters (v2.0)
  // S_obs_max: supremum of observer entropy over compact K (Def. 3.4).
  // Stability is coupled to entropy algebraically (Corollary 3.7):
  //   Ŝ_k = 1 − Ĥ_k / S_obs_max  (no independent SDE)
  S_obs_max: 1.648,  // = H_c * 1.5
  S_c: 0.7,
  ...
  // Noise (Assumption 4.1(ε')) — entropy channel only;
  // stability carries no independent noise term in v2.0.
  sigma_H: 0.04,
...
function simulate(params, useGovernor) {
  ...
  for (let k = 0; k <= p.T; k++) {
    ...
    // Stability update — algebraic coupling (Corollary 3.7):
    // Ŝ_k = 1 − Ĥ_k / S_obs_max (no independent SDE for stability)
...
function PhaseDiagram({ controlled, uncontrolled, params }) {
  ...
  // Algebraic coupling manifold (Corollary 3.7): Ŝ = 1 − Ĥ / S_obs_max
  // Trajectories in v2.0 are constrained to lie on this curve — drawn
  // as a dotted line for reference, mirroring the ls=":" manifold plot
  // in simulator.py's panel (F).
  const manifold = useMemo(() => {
    const N = 200;
    const pts = [];
    for (let i = 0; i <= N; i++) {
      const H = (p.S_obs_max * i) / N;
      pts.push({ H, S: Math.max(0, Math.min(1, 1 - H / p.S_obs_max)) });
    }
    return pts;
  }, [p.S_obs_max]);
    ...
    <div style={{ position: "relative" }}>
      <ResponsiveContainer width="100%" height={340}>
        <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
          ...
          <Line data={manifold} dataKey="S" name="Algebraic manifold (v2.0)" stroke="#94a3b8"
...
export default function CPLDashboard() {
  ...
  // σ_eff = σ_H (Lemma 4.3): in v2.0, stability is algebraically coupled
  // to entropy (Corollary 3.7) and carries no independent noise term, so
  // ξ_{k+1} = −ε_k^H · 𝟙{Ĥ_k > H_c} is driven by the entropy channel alone.
  const V0 = uncontrolled[0]?.V || 0.1;
  const delta_drift = 0.05;
  const sigma_eff = params.sigma_H;
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 12, color: "#64748b", margin: "6px 0 0 0", maxWidth: 700 }}>
          Discrete simulation of Eqs. (9), (14) and algebraic coupling (Corollary 3.7) with policy §4.
      ...
      {showSettings && (
        <div style={{
          ...
          {[
            ...
            { key: "S_obs_max", label: "S_obs_max (entropy sup.)", min: 1.0, max: 3.0, step: 0.01 },
      ...
      <div>
        ...
        {activeTab === "fragmentation" && (
          <div style={chartStyle}>
            ...
            <div style={{ fontSize: 11, color: "#64748b", marginBottom: 12, paddingLeft: 8 }}>
              Empirical N_F stays below the √T bound at δ₀ = 0.05. σ_eff = σ_H = {sigma_eff.toFixed(4)} (Lemma 4.3; stability carries no independent noise in v2.0).
            </div>
            <ResponsiveContainer width="100%" height={320}>
              <ComposedChart margin={{ top: 5, right: 30, bottom: 5, left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.08)" />
      ...
      <div style={{ marginTop: 24, fontSize: 10, color: "#475569", lineHeight: 1.6, borderTop: "1px solid rgba(148,163,184,0.08)", paddingTop: 12 }}>
        ...
        <div>Information-Geometric Context Window Governance (CPL Framework) · doi:10.5281/zenodo.22056414</div>
        <div style={{ marginTop: 4 }}>
          Simulation implements discrete Eqs. (9), (14) and algebraic coupling (Corollary 3.7) with policy (18)–(19).
