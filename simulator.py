...
Key changes relative to the v1.0 simulator
-------------------------------------------
[FIX-1]  DOI / citation updated to v2.0  (doi:10.5281/zenodo.19177363).

[FIX-2]  Stability Ŝ_k is no longer an independent stochastic process.
         Per Definition 3.5 and Corollary 3.7 of the paper, stability is
         an algebraic consequence of entropy:

             Ŝ_k = 1 - Ĥ_k / S_obs_max

         The v1.0 parameters  ρ, b_C, b_R, b_F, κ_L, κ_P, σ_S  that
         governed an independent stability SDE are removed.  All noise is
         carried by the entropy channel  σ_H  only (Assumption 4.1(ε')).

[FIX-3]  sweep_drift_boundary() and the --sweep CLI flag now reference
         Lemma 4.4 (Revised Drift Conditions) + Assumption 3.14
         (Spectral Response), which replaced the unsatisfiable
         Lemma 5.3(ii) of v1.0.  The sweep explores the boundary of
         Assumption 3.14 by varying  α_tight  and  Q_ratio  (the ratio
         Q(θ_tight)/Q(θ_base)).

[FIX-4]  Plot titles and axis annotations reference v2.0 proposition /
         theorem numbers:
           • Prop. 5.1  → Prop. 5.1  (unchanged)
           • Prop. 5.2  → Prop. 5.2  (unchanged)
           • Thm. 5.4   → Thm. 5.3   (fragmentation bound in v2.0)

[FIX-5]  Standing-hypothesis comment updated from "Assumption 3.1"
         (v1.0 numbering) to "Assumption 4.1 (U), (Y)" (v2.0).

[FIX-6]  h_target() now accepts θ as an explicit parameter consistent
         with  h(L, θ_tight)  in Assumption 4.1(h) of the paper.
         At runtime the caller always passes  θ^tight ; the signature
         makes the dependence visible and keeps the code aligned with
...
class CPLConfig:
    ...
    Parameters removed relative to v1.0 (all belonged to the independent
    stability SDE, which is replaced by the algebraic coupling in v2.0):
        rho, b_C, b_R, b_F, kappa_L, kappa_P, sigma_S

    New parameters (v2.0 information-geometric grounding):
    ...
    def h_target(self, L: float, theta: str = "tight") -> float:
        ...
        theta : decoding mode ('tight' or 'base'); included per the
                formal signature  h(L, θ)  in the paper [FIX-6].
...
def simulate(cfg: CPLConfig, use_governor: bool) -> list[StepRecord]:
    ...
    S: float = 1.0 - H / cfg.S_obs_max          # algebraic stability [FIX-2]
    H_prev: float = H
    z_prev: str   = "C"
...
def theoretical_bound(
    ...
    In v2.0  σ = σ_H  (scalar, since stability noise is removed).  [FIX-4]
    """
...
def plot_all(cfg: CPLConfig, outdir: Path) -> Path:
    ...
    Panel references updated to v2.0 proposition/theorem numbers [FIX-4]:
      ...
      (E) N_F(T) — Thm. 5.3  (was Thm. 5.4 in v1.0)
      (F) Phase diagram (Ĥ, Ŝ)
    """
    ...
    # ── (A) Context Length ────────────────────────────────────────────
    # Prop. 5.1 numbering unchanged in v2.0  [FIX-4: no change needed here]
    ...
    # ── (B) Observer Entropy ──────────────────────────────────────────
    # Prop. 5.2 numbering unchanged in v2.0  [FIX-4: no change needed here]
    ...
    # v2.0: Ŝ_k = 1 − Ĥ_k / S_obs_max  (Corollary 3.7)  [FIX-2]
    ...
    # Theorem 5.3 in v2.0  (was Thm. 5.4 in v1.0)  [FIX-4]
...
# ══════════════════════════════════════════════════════════════════════
# Spectral-response boundary sweep  (replaces v1.0 "Lemma 5.3 sweep")
# ══════════════════════════════════════════════════════════════════════

def sweep_spectral_boundary(outdir: Path) -> None:
    """
    Sweep the boundary of Assumption 3.14 (Spectral Response Under Tight
    Decoding) in v2.0, which replaced the unsatisfiable Lemma 5.3(ii)
    of v1.0.  [FIX-3]
    ...
    fig.suptitle(
        ...
        "replaces Lemma 5.3(ii) sweep from v1.0  [FIX-3]",
...
def main() -> None:
    ...
    print(f"  σ_eff           : {cfg.sigma_eff:.4f}"
          f"  (= σ_H only; σ_S removed in v2.0)")
