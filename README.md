...
> *Note on `simulator.py` / dashboard: `simulator.py` in the repository root implements the v2.0 governor model described in Section 5 of the paper (algebraic stability coupling, Corollary 3.7). The interactive dashboard, however, has not yet been verified against this v2.0 model as of the current update; until reconciled, treat the paper itself (Section 5, "Control-Theoretic Response: CPL 4.0 Governor") as the authoritative specification of the governor equations in case of any discrepancy with dashboard behavior.*
